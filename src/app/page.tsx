...
export default function App() {
  ...
  const addEntry = () => {
    ...
    setEntries((prev) =>
      [...newOnes, ...prev].sort((a, b) => (a.date < b.date ? 1 : -1))
    );
    setForm((f) => ({ ...f, note: "" }));
    toast("追加されました！");
  };
  ...
  const undoDelete = () => {
    ...
    setEntries((prev) =>
      [...lastDeleted, ...prev].sort((a, b) => (a.date < b.date ? 1 : -1))
    );
    setLastDeleted(null);
    toast("削除を取り消しました");
  };
  ...
  const daySummary = useMemo(() => {
    ...
    for (const e of list) {
      if (!map.has(e.liftId)) map.set(e.liftId, []);
            ...
            <Card className="shadow-sm">
              <CardContent className="p-4 grid grid-cols-2 md:grid-cols-6 gap-3 md:items-center">
                ...
                <div>
                  ...
                  <Select
                    value={fLift}
                    onValueChange={(v) => setFLift(v as LiftFilter)}
                  >
                    <SelectTrigger className="h-9">
                      <SelectValue placeholder="すべて" />
                    </SelectTrigger>
            ...
            <Card className="shadow-sm">
              <CardContent className="p-4 grid grid-cols-2 md:grid-cols-8 gap-3 items-center">
                ...
                <div>
                  ...
                  <Select
                    value={form.part}
                    onValueChange={(v) => setForm({ ...form, part: v as Part })}
                  >
                    <SelectTrigger className="h-9">
                ...
                <div className="col-span-2">
                  ...
                  <Select
                    value={form.liftId}
                    onValueChange={(v) =>
                      setForm({ ...form, liftId: v as LiftId })
                    }
                  >
                    <SelectTrigger className="h-9">
                      <SelectValue placeholder="選択" />
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setForm({
                        ...form,
                        weight: Number.isFinite(toNumber(e.target.value))
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setForm({
                        ...form,
                        reps: Number.isFinite(toNumber(e.target.value))
                          ? toNumber(e.target.value)
                          : 0,
                      })
                    }
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setForm({
                        ...form,
                        sets: Number.isFinite(toNumber(e.target.value))
                          ? toNumber(e.target.value)
                          : 0,
                      })
                    }
...
function LogTable({
  ...
  const nameOfLocal = (id: LiftId) =>
    lifts.find((l) => l.id === id)?.name || id;

  const groups = useMemo(() => {
    const m = new Map<string, SetEntry[]>();
    for (const e of entries) {
      const k = `${e.date}|${e.liftId}`;
      if (!m.has(k)) m.set(k, []);
      m.get(k)!.push(e);
    }
                ...
                <React.Fragment key={key}>
                  <tr className="bg-neutral-100">
                    <td colSpan={6} className="p-2 font-medium">
                      {date} / {nameOfLocal(liftId as LiftId)}{" "}
                  ...
                  {arr.map((e, i) => (
                    ...
                    >
                      ...
                      <td className="p-2 whitespace-nowrap">
                        {nameOfLocal(e.liftId)}
                      </td>
                      <td className="p-2">{e.weight}</td>
                      <td className="p-2">{e.reps}</td>
                      <td className="p-2 max-w-[400px] truncate" title={e.note}>
                        {e.note}
                      </td>
                    </motion.tr>
                  ))}
    ...
    <Card className="shadow-sm">
      <CardContent className="p-4 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-[1fr_160px_120px] gap-2">
          <Input
            className="h-9"
            placeholder="種目名（例: Pull-up）"
            value={newLiftName}
            onChange={(e) => setNewLiftName(e.target.value)}
          />
          <Select
            value={newLiftPart}
            onValueChange={(v) => setNewLiftPart(v as Part)}
          >
            <SelectTrigger className="h-9">
        ...
        <div className="grid grid-cols-1 md:grid-cols-[1fr_160px_120px_120px] gap-2">
          ...
          <Select
            value={editLiftPart}
            onValueChange={(v) => setEditLiftPart(v as Part)}
          >
            <SelectTrigger className="h-9">
              <SelectValue placeholder="部位" />
            </SelectTrigger>
